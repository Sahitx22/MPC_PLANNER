...
void updateTF()
{
  while (true)
  {
    try
    {
      listener_ptr->waitForTransform("world_r", "base_link_r", ros::Time(0), ros::Duration(0.1));
      listener_ptr->lookupTransform("world_r", "base_link_r", ros::Time(0), base_link_transform);
      listener_ptr->waitForTransform("world_r", "os_sensor", ros::Time(0), ros::Duration(0.1));
      listener_ptr->lookupTransform("world_r", "os_sensor", ros::Time(0), lidar_link_transform);
      break;
    }
...
int main(int argc, char **argv)
{
  ...
  velodyne_sub = nh.subscribe<sensor_msgs::PointCloud2>("/pfx", 1, [&](sensor_msgs::PointCloud2::ConstPtr msg)
                                                        { 
  ...
  while (ros::ok())
  {
    ...
    local_velodyne_msg.header.frame_id = "world_r";
