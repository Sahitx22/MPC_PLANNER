...
class Local_Planner():
    ...
    def __publish_local_plan(self, input_sol, state_sol):
        ...
        local_path.header.frame_id = "world_r"
        local_path_vis.header.frame_id = "world_r"
        ...
        for i in range(self.N):
            ...
            this_pose_stamped.header.frame_id = "world_r"
    ...
    def MPC_ellip(self):
        ...
        for j in range(num_obs):
            if not exceed_ob(self.ob[50*j]):
                for i in range(self.N-1):
                    opti.subject_to(h(opt_states[i + 1, :], self.ob[j * 50 + i + 1]) >=
                                    (1 - gamma_k) * h(opt_states[i, :], self.ob[j * 50 + i]))
